...
export async function getDailyBonusStatus(nickname: string): Promise<DailyBonusStatus> {
  const result = await query(
    `SELECT coins, daily_streak, last_bonus_at, total_bonus_claimed
     FROM players WHERE nickname = $1`,
    [nickname]
  );
  ...
  const row: DailyBonusRow = result.rows[0];
  const now = new Date();
...
export async function claimDailyBonus(nickname: string): Promise<DailyBonusClaimResult> {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    ...
    const result = await client.query(
      `SELECT coins, daily_streak, last_bonus_at, total_bonus_claimed
       FROM players WHERE nickname = $1 FOR UPDATE`,
      [nickname]
    );
    ...
    await client.query(
      `UPDATE players
       ...
       WHERE nickname = $4`,
      [newCoins, newStreak, now.toISOString(), nickname]
    );
