import { Card, GameState, PlayerState, RuleVariants, TrickCard, RANK_ORDER } from '../types/game';
...
export class RulesEngine {
  ...
  static rankValue(rank: string): number {
    return RANK_ORDER.indexOf(rank as any);
  }
  ...
  static isPointCard(card: Card, rules: RuleVariants): boolean {
    ...
    if (card.id === 'Q_spades') return true;
    if (rules.blackMaria) {
      if (card.id === 'K_spades' || card.id === 'A_spades') return true;
    }
    return false;
  }
  ...
  static doesBreakHearts(card: Card, rules: RuleVariants): boolean {
    if (card.suit === 'hearts') return true;
    if (rules.queenBreaksHearts && card.id === 'Q_spades') return true;
    return false;
  }
  ...
  static calculateRoundScores(
    ...
    for (const player of players) {
      ...
      for (const card of allCards) {
        ...
        if (rules.blackMaria) {
          if (card.id === 'K_spades') score += 10;
          if (card.id === 'A_spades') score += 7;
        }
      }
  ...
  static resolvePassDirection(rules: RuleVariants, round: number): 'left' | 'right' | 'across' | 'none' {
    ...
    if (dir === 'random') {
      const options: ('left' | 'right' | 'across' | 'none')[] = ['left', 'right', 'across', 'none'];
    ...
    if (dir === 'left' || dir === 'right' || dir === 'across' || dir === 'none') {
      ...
      if (dir === 'left') {
        const cycle: ('left' | 'right' | 'across' | 'none')[] = ['left', 'right', 'across', 'none'];
